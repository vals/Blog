...
def main():
    ...
    # Create model with one-hot batch representation (default)
    print(f"Creating model with {batch_method} batch representation...")
    model = scvi.model.SCVI(
        adata_train,
        n_layers=1,
        batch_representation='one-hot',  # Default one-hot batch representation
        gene_likelihood='nb'  # Note: batch_embedding_kwargs and compute_pseudobulk not used with one-hot
    )
    ...
    training_log = {
        ...
        'history': {
            'elbo_train': model.history['elbo_train'].values.tolist(),
            'elbo_validation': model.history['elbo_validation'].values.tolist()
        }
    }
