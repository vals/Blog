...
def main():
    ...
    # Create model with embedding batch representation
    print(f"Creating model with {batch_method} batch representation...")
    model = scvi.model.SCVI(
        adata_train,
        n_layers=1,
        batch_representation='embedding',  # Changed from 'variational' to 'embedding'
        batch_embedding_kwargs={"embedding_dim": 5},
        gene_likelihood='nb'  # Note: compute_pseudobulk not used with embedding method
    )
