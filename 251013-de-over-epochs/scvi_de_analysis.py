...
This script trains an SCVI model epoch-by-epoch and performs differential
expression analysis after each epoch to visualize how DE results evolve.
"""
...
print(f"Observations columns: {adata.obs.columns.tolist()}")
print(f"\nUnique conditions: {adata.obs['condition'].unique()}")
print(f"\nCell type subsets: {adata.obs['celltype_subset'].unique()}")
...
# Explore group column to find tissue information
print("\nExploring 'group' column:")
print(adata_paired.obs['group'].value_counts())

# Count cells per cell type and group
print("\nCounting cells per cell type and group...")
cell_counts = adata_paired.obs.groupby(['celltype_subset', 'group']).size().unstack(fill_value=0)
print(cell_counts)
...
print(f"\nValid cell types (≥50 cells in both Inflamed and NonInvolved): {valid_celltypes}")
...
def perform_de_analysis(model, adata_paired, valid_celltypes):
    ...
    Returns a dictionary mapping cell types to DE results DataFrames.
    """
    de_results = {}
    ...
    for celltype in valid_celltypes:
        ...
        # Perform DE test: Inflamed vs NonInvolved
        try:
...
# ============================================================================
# Training Loop with Epoch-by-Epoch DE Analysis
# ============================================================================
print("\n" + "="*80)
print("Starting epoch-by-epoch training with DE analysis...")
print("="*80)

# Training parameters
max_epochs = 100
check_interval = 5  # Perform DE analysis every N epochs
...
for epoch in range(1, max_epochs + 1):
    # Train for one epoch with 90% train / 10% validation split
    model.train(
        max_epochs=1,
        train_size=0.9,
    ...
    # Perform DE analysis at intervals
...
print(f"  ffmpeg -framerate 10 -pattern_type glob -i '{PLOT_DIR}/volcano_epoch_*.png' -c:v libx264 -pix_fmt yuv420p {OUTPUT_DIR}/de_evolution.mp4")
