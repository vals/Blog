...
"""
Simulate gameplay using a naive "always cash out" strategy.
...
class NaiveGameplaySimulator:
    ...
    def simulate_episode(self, starting_balance: int, run_id: int = 0, episode_id: int = 0) -> Tuple[int, Dict]:
        """
        Simulate a single episode using the naive "always cash out" strategy.
        ...
        # Naive strategy: Always cash out immediately (action 8)
        action = 8  # Cash out action
        ...
        episode_stats = {
            ...
            'episode_length': 1,  # Always 1 step for naive strategy
    ...
    def _record_decision_data(self, run_id: int, episode_id: int, action: int, balance: int, step: int):
        ...
        # Get valid actions (for naive strategy, we only consider cash out)
        valid_actions = [8]  # Always only cash out for naive strategy
    ...
    def simulate_session(self, initial_balance: int = 100, max_episodes: int = 500, verbose: bool = True, run_id: int = 0) -> Dict:
        ...
        """
        if verbose:
            print(f"🎰 Starting naive strategy simulation:")
            print(f"   Initial balance: {initial_balance} Gold")
            print(f"   Maximum episodes: {max_episodes}")
            print(f"   Strategy: Always cash out immediately")
            print("-" * 50)
        ...
        if verbose:
            print("\n" + "="*50)
            print("🎯 NAIVE SIMULATION COMPLETED")
            print("="*50)
    ...
    def save_simulation_data(self, filename: str = "naive_slot_machine_data.csv"):
        ...
        print(f"📊 Naive simulation data saved to {filename}")
    ...
    def simulate_multiple_sessions(self, num_runs: int = 25, initial_balance: int = 100, max_episodes: int = 500) -> List[List[int]]:
        ...
        """
        print(f"🎰 Running {num_runs} independent naive strategy sessions:")
        ...
        for run in tqdm(range(num_runs), desc="Running naive simulations"):
            # Reset for new run
            self.balance_history = []
            self.episode_stats = []
    ...
    def _print_aggregate_stats(self, all_session_stats: List[Dict], initial_balance: int):
        ...
        print("\n" + "="*60)
        print("📊 AGGREGATE NAIVE STRATEGY RESULTS")
        print("="*60)
    ...
    def plot_multiple_balance_histories(self, all_balance_histories: List[List[int]], 
                                      ...
                                      filename: str = "naive_balance_simulation.png"):
        ...
        ax1.set_title(f'Naive Strategy Balance Progression - {len(all_balance_histories)} Independent Runs', 
        ...
        stats_text = f"""Naive Strategy Statistics:
...
Median Final: {np.median(final_balances):.1f}"""
        ...
        plt.close()
        print(f"📊 Naive strategy balance progressions saved to {filename}")

def main():
    """Main simulation function."""
    print("🎰 Naive Slot Machine Strategy Simulation - Multiple Runs")
    print("=" * 60)
    print("Strategy: Always cash out immediately (no respins)")
    print("=" * 60)
    
    # Initialize simulator
    simulator = NaiveGameplaySimulator()
    
    print("\nRunning 25 independent 500-episode sessions...")
    print("This will show the baseline performance of immediate cash-out strategy.")
    print()
    ...
    with open("naive_aggregate_results.txt", 'w') as f:
        f.write("NAIVE SLOT MACHINE STRATEGY - AGGREGATE SIMULATION RESULTS\n")
        f.write("=" * 60 + "\n\n")
        f.write("Strategy: Always cash out immediately (no respins)\n\n")
    ...
    print("\n🎯 Naive strategy simulation complete! Check the generated files:")
    print("   📊 naive_balance_simulation.png - All 25 balance progression curves")
    print("   📄 naive_aggregate_results.txt - Comprehensive statistics")
    print("   📊 naive_slot_machine_data.csv - Detailed decision-by-decision data")
    print(f"\nThis analysis shows the baseline performance of always cashing out,")
    print(f"providing a comparison point for more sophisticated strategies.")
