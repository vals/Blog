"""
Plot training curves from scVI batch size benchmark.
"""
import pandas as pd
...
# Load results
results_df = pd.read_csv(f"{OUTPUT_DIR}/benchmark_results.csv")
...
print("\nTiming summary:")
timing = results_df.groupby('batch_size')['total_training_time'].first()
for bs, t in timing.items():
    print(f"  Batch size {bs}: {t:.1f}s")
